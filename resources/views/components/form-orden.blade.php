...
<div class="col-sm-5">
    ...
    <select class="form-select" name="codigo_producto" id="CodigoProducto">
        ...
        @foreach($productos as $producto)
            <option value="{{$producto->codigo_producto}}" @selected(old('codigo_producto', $orden->codigo_producto) == $producto->codigo_producto)>{{$producto->codigo_producto}}</option>
        @endforeach
    </select>
</div>
...
<div class="col-sm-3">
    ...
    <select class="form-select" name="unidad_de_medida" id="UnidadMedida">
        <option selected disabled value="0">Seleccione...</option>
        @foreach($productos as $producto)
            <option value="{{$producto->unidad_de_medida}}" @selected(old('unidad_de_medida', $orden->unidad_de_medida) == $producto->unidad_de_medida)>{{$producto->unidad_de_medida}}</option>
        @endforeach
    </select>
</div>
